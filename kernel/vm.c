...
int dfs_vmprint(pagetable_t pagetable, int level) {
    for(int i = 0; i < 512; ++i) {
    pte_t pte = pagetable[i];
    if((pte & PTE_V) == 0)
      continue;
    ...
    for(int j = 0; j < level; ++j)
      printf(" ..");
    pte_t nxt = PTE2PA(pte);
    printf("%d: pte %p pa %p\n", i, pte, nxt);
    if((pte & (PTE_R|PTE_W|PTE_X)) == 0)
        dfs_vmprint((pagetable_t)nxt, level+1);
  }
  return 0;
}
...
int vmprint(pagetable_t pagetable) {
  printf("page table %p\n", pagetable);
  return dfs_vmprint(pagetable, 0);
}

pagetable_t kvmalloc() {
...
int kvmcopy_fromuvm(pagetable_t old, pagetable_t new, uint64 start, uint64 sz) {
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  ...
  for(i = PGROUNDUP(start); i < start + sz; i += PGSIZE) {
    if((pte = walk(old, i, 0)) == 0)
      panic("kvmcopy_fromuvm: pte should exist");
    if((*pte & PTE_V) == 0)
      panic("kvmcopy_fromuvm: page not present");
    pa = PTE2PA(*pte);
